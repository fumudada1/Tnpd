...
namespace Tnpd.Models
{
    public class Utility
    {
        ...
        public static void GetPerssionGlobal(TnpdModels.Member member)
        {
        ...
        public static string GetPermissionString(string permission, string fileName = "~/Config/Menu.xml")
        {
        ...
        private static void GetPermissionScript(XmlNode ccNode, string permission)
        {
            foreach (XmlNode cnode in ccNode.ChildNodes)
            {
        ...
        private static void GetMenuValue(string controllerName, XmlNode node, Member member)
        {
            ...
            if (node.Attributes["pclass"] != null)
            {
                _strMenuBuilder.Append("<a href=\"" +
                                       urlHelper.Action(node.Attributes["Action"].Value,
                                           node.Attributes["Controller"].Value, new { pclass = node.Attributes["pclass"].Value }) +
            ...
            else
            {
                _strMenuBuilder.Append("<a href=\"" +
                                       urlHelper.Action(node.Attributes["Action"].Value,
                                           node.Attributes["Controller"].Value) +
                                       "\" data-target-page=\"page-dashboard\">\n");
            }
        ...
        private static void GetSubMenuValue(string controllerName, UrlHelper urlHelper, XmlNode node, Member member)
        {
            ...
            if (node.HasChildNodes)
            {
                if (node.Attributes["IsNoChild"] != null && node.Attributes["IsNoChild"].Value.ToLower().IndexOf("true", System.StringComparison.Ordinal) > -1)
                {
                }
                else
                {
            ...
            {
                _strMenuBuilder.Append("<a href=\"" +
                                       urlHelper.Action(node.Attributes["Action"].Value, node.Attributes["Controller"].Value, new { pclass = node.Attributes["pclass"].Value }) +
                                       "\" data-target-page=\"page-dashboard\">\n");
            }
            else
            {
        ...
        {
            string[] str = HttpContext.Current.User.Identity.Name.Split('\\');
            //return HttpContext.Current.User.Identity.Name.Split('\\')[1];
            return str.Length > 1 ? str[1] : str[0];
        ...
        static public string SaveUpImage(HttpPostedFileBase upfile)
        {
            ...
            string savedName1 = Path.Combine(HttpContext.Current.Server.MapPath("~/upfiles/images"), "M" + fileName);
            upfile.SaveAs(savedName);
            upfile.SaveAs(savedName1);
            return fileName;
        }
        #endregion
        ...
        /// <param name="maxHight">指定要縮的高度</param>
        /// <remarks></remarks>
        static public void GenerateThumbnailImage(string name, Stream source, string target, string suffix, int maxWidth, int maxHight)
        {
            Image baseImage = Image.FromStream(source);
            Single ratio = 0.0F; //存放縮圖比例
            Single h = baseImage.Height; //圖像原尺寸高度
            Single w = baseImage.Width;  //圖像原尺寸寬度
            int ht; //圖像縮圖後高度
        ...
        /// <param name="MaxHight">指定要縮的高度</param>
        /// <remarks></remarks>
        static public void GenerateThumbnailImageGinny(string name, System.IO.Stream source, string target, string suffix, int MaxWidth, int MaxHight)
        {
        ...
        /// 透過md5加密
        /// </summary>
        /// <param name="cryString"></param>
        /// <returns></returns>
        static public string EecodingByMD5(string original)
        {
            string key = "cypdcypd";
            string iv = "04130413";
        ...
        #region "解密函式"
        static public string DecodingByMD5(string hexString)
        {
            string key = "cypdcypd";
            string iv = "04130413";
        ...
        #region "發信"
        public static void SystemSendMail1(string toAddress, string subject, string mailBody)
        {
        ...
        #region "檢查HTML 是否符合無障礙(檢查alt 與summery)"

        public static string ChkAccessible(string article)
        {
            if (ConfigurationManager.AppSettings["OpenAccessible"] == "false")
            {
                return "success";
            }
            ...
            if (tableNodes != null)
            {
                foreach (HtmlNode img in doc.DocumentNode.SelectNodes("//table"))
                {
                    if (!img.Attributes.Contains("summary"))
                    {
                        return "表格需要填寫摘要";
