...
namespace tnpd.Controllers
{
    public class FAQController : _BaseController
    {
        ...
        public ActionResult Index(Guid id, int? page, FormCollection fc)
        {
            ...
            var newses = _db.Newses.Where(x => x.NewsCatalogs.Count(y => y.WebSiteId == 1) > 0 && x.NewsCatalogs.Count(y => y.WebCategoryId == 58) > 0).OrderByDescending(p => p.InitDate).AsQueryable();
            ...
            var CategoryCount = _db.NewsCatalogs.Count(x => x.WebCategoryId == 58 && x.WebSiteId == 1);
